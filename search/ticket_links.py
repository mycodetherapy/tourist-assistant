...
def _avia_aviasales(
    ...
    if origin_iata and dest_iata and dep:
        url = build_aviasales_search_url(origin_iata, dest_iata, dep, ret)
    else:
        q = urlencode(
            {
                "origin": origin,
...
def _train_rzd(
    ...
    dest: str,
    dep: date | None,
    travelers: str = DEFAULT_TRAVELERS,
) -> TicketOffer | None:
    ...
    return _offer(
        ...
        provider="РЖД",
        label=f"РЖД: {origin} → {dest}, {_fmt_tutu_date(dep)}",
        url=url,
    )
...
def build_ticket_offers(
    ...
    rzd = _train_rzd(origin_city, destination_city, dep)
    if rzd:
        offers.append(rzd)
    tutu_train = _train_tutu(origin_city, destination_city, dep)
    if tutu_train:
        offers.append(tutu_train)

    bus = _bus_tutu(origin_city, destination_city, dep)
    if bus:
        offers.append(bus)
