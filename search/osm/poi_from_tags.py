...
from search.yandex.poi_filters import is_landmark_poi_name
...
def _pick_name(tags: dict[str, str], *, city_hint: str = "") -> str:
    for key in ("name:ru", "name", "official_name", "alt_name:ru", "alt_name"):
...
def infer_tag_from_osm_tags(tags: dict[str, str]) -> LeisureTag:
    ...
    if "набереж" in _pick_name(tags).lower():
        return "embankments"
    if building in {"cathedral", "church", "chapel", "mosque", "synagogue"}:
        return "landmarks"
...
def osm_element_to_poi(
    ...
    *,
    ...
    if not isinstance(tags, dict):
        return None
    name = _pick_name(tags, city_hint=city_hint)
    if not name or not is_landmark_poi_name(name, city_hint=city_hint):
        return None
    lat = lon = None
    ...
    tag = infer_tag_from_osm_tags({str(k): str(v) for k, v in tags.items()})
    address = str(tags.get("addr:full") or tags.get("addr:street") or "").strip()
    return PoiPoint(
        poi_id=poi_id,
        tag=tag,
        name=name,
