...
export const FREE_VS_LLM = {
  ...
  cityPackHint:
    "Расширенный справочник мест подключается только с LLM, если город загружен на сервер.",
} as const;
...
export const HOW_ROUTES_WORK_SECTIONS = [
  ...
  {
    title: "Карта",
    body: "Линия по улицам появляется, когда для города есть пеший граф на сервере. Иначе остаётся виджет Яндекс.Карт. Готовой картой можно пользоваться в любом режиме. Подготовить новый город — в настройках (бесплатно до 3; со своим API-ключом без лимита).",
  },
] as const;
